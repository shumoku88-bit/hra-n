...
| Capacity/Budget | separate capacity plane, per-movement effective evidence, non-negative purpose guard; universal capacity conservation sum = 0 | `capacity.loam` canonical authority with atomic lock/stage/rename; no legacy Capacity read/write or one-shot Budget read fallback; report tabs still have a separate legacy projection entrance | `Capacity_Query` and one-shot `Budget_Query.Execute[_Window]` canonical-only, `Loam_Capacity_Writer`; legacy `Budget_Query.Project` is still called by transitional reports | `hra-n capacity [transfer/rebalance]` and `hra-n budget [START] [END]` | Capacity and Budget workspaces read-only, Financial report TUI budget view remains transitional | conservation law verification, wire-admission, E2E CLI and PTY refusal tests; SPARK core | **Canonical Capacity/one-shot Budget; legacy report projection still present** |
