...
| Capacity/Budget | separate capacity plane, per-movement effective evidence, non-negative purpose guard, effective-only consumption; window stays a query coordinate | append-only TRANSFER/REBALANCE/EFFECTIVE facts via generation transaction | `Capacity_Command` transfer/rebalance Intents -> snapshot-bound Proposal -> Receipt; minimal `Capacity_Query` readout | `hra-n capacity` / `capacity transfer` / `capacity rebalance` | missing (Capacity workspace and current-cycle Budget decision surface pending) | transfer/rebalance-law, stale-proposal, retry, wire-admission, E2E CLI, and Alloy capacity shape model | **Legacy** |
