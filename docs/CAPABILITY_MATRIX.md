...
| Accounting roles | direct read of `accounting-role.loam` and `locus-admission.loam`; legacy policy.hra fallback | `accounting-role.loam` canonical authority | `Policy_Query.Execute_Role_Query` (direct Loam loader + legacy fallback) | `hra-n role [list/--as-of]` | Home count and Role inspection | canonical reader unit tests, CLI tests; SPARK core | **Canonical read slice; writer blocked** |
...
| Policy administration | versioned role/routing facts with effective coordinates, and explicit add-only Locus admission vocabulary | `locus-admission.loam` and `accounting-role.loam` canonical authorities | `Policy_Query.Execute_Locus_Query` and `Policy_Command` | `hra-n locus list`, `hra-n role` | Locus workspace (`v`), routing workspace (`r`) | unit, E2E CLI, PTY, and formal tests | **Canonical read slice; writer blocked** |
