...
| Capacity/Budget | separate capacity plane, per-movement effective evidence, non-negative purpose guard; universal capacity conservation sum = 0 | `capacity.loam` canonical authority with atomic lock/stage/rename; no public legacy Capacity/Budget/report read entrance | `Capacity_Query` and one-shot `Budget_Query.Execute[_Window]` canonical-only, `Loam_Capacity_Writer`; `Budget_Query.Project` still accepts transitional in-memory types adapted from canonical bytes | `hra-n capacity [transfer/rebalance]` and `hra-n budget [START] [END]` | Capacity and Budget workspaces read-only; report TUI has canonical-only entrance | conservation law verification, wire-admission, E2E CLI and PTY refusal tests; SPARK core | **Canonical Capacity/Budget read entrance; transitional internal projection types** |
...
- F08 legacy report entrance removed: `report` CLI (including `--statement`)
  and all seven TUI tabs reject a legacy-only root. The transitional Application
