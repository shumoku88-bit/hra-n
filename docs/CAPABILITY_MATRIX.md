...
| Policy administration | versioned role/routing facts with effective coordinates; fail-closed admission via core and Alloy laws | append-only policy.hra via generation transaction with exact selected-candidate comparison | `Policy_Command` (Propose_Role, Propose_Window, Propose_Routing, Commit) | `hra-n role assign`, `hra-n window add`, `hra-n route set/clear` | Actual routing workspace (`r`) | unit, proposal/commit/stale/retry, E2E CLI, PTY, and formal tests | **V2** |
