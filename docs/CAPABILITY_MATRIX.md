...
| Actual list by day/all | occurrence date and stable source order | canonical `actual.loam` for direct CLI; shared TUI Query still has transitional legacy fallback | `Actual_Query` (canonical direct read and TUI selection) | `hra-n actual FILE [DATE]`; legacy journal `review` removed (search/week not claimed) | Home, Selected Day, Actual (canonical where selected) | canonical reader/unit/CLI + PTY; remaining Home/Actual legacy tests pending cutover | **Canonical direct CLI; shared TUI cutover incomplete** |
