...
| Capacity/Budget | separate capacity plane, per-movement effective evidence, non-negative purpose guard; universal capacity conservation sum = 0 | `capacity.loam` canonical authority with atomic lock/stage/rename | `Capacity_Query`, `Budget_Query`, and `Loam_Capacity_Writer` | `hra-n capacity [transfer/rebalance]` and `hra-n budget [START] [END]` | Capacity and Budget workspaces, Financial report TUI budget view | conservation law verification, wire-admission, E2E CLI and PTY tests; SPARK core | **Canonical write slice** |
...
| Attention | retained matters with three due meanings, one closure each; provenance never closes | read-only `attention.loam` admission; legacy read fallback remains pending removal | `Attention_Query` independently selects canonical/legacy/unavailable; legacy `Attention_Command` deleted | list only; all mutation subcommands explicitly reject | read-only workspace; legacy editors deleted; Home shares open observation | CLI and PTY source/refusal tests; SPARK Core | **Canonical read slice; writer absent** |
