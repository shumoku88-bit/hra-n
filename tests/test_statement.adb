...
package body Test_Statement is
   ...
   begin
      ...
      --  An empty root must not fabricate a Statement from missing evidence.
      declare
         Rep : constant Statement_Report := Execute_Statement_Query (Paths);
      begin
         Assert (Rep.Status = Query_Rejected
                 and then Rep.Diagnostic (1 .. Rep.Diagnostic_Len) =
                   "canonical statement evidence required",
                 "legacy-only Statement refuses without a canonical authority");
      end;
      ...
      begin
         Assert (Rep.Status = Query_Rejected
                 and then Rep.Total_Events = 0
                 and then Rep.Diagnostic (1 .. Rep.Diagnostic_Len) =
                   "canonical statement evidence required",
                 "valid old Policy and Journal cannot satisfy public Statement");
      end;
      Assert (Write_File_Atomically
