...
package body HRA_N.UI.Capacity_TUI is
   ...
   --  Minimal curses line editor. Echoes input on Prompt_Row, accepts on
   --  Enter, cancels on Esc. Returns an empty string only when the caller
   --  allows blank lines or the user cancels (Cancelled).
   procedure Edit_Line
     (Prompt_Row   : Natural;
      Prompt_Text  : String;
      Initial      : String;
   ...
   begin
      Result (Result'First .. Result'Last) := [others => ' '];
      Result_Len := 0;
      Cancelled := False;
   ...
   function Prompt_For
     (Prompt_Row  : Natural;
      Prompt_Text : String;
      Initial     : String := "";
      Allow_Blank : Boolean := False) return String
   is
      Buf : String (1 .. Max_Token_Length) := [others => ' '];
      Len : Natural;
      Cancelled : Boolean;
   ...
   begin
      ...
      if Rows > 2 then
         ...
         Put_Clipped
           (Rows - 2,
            "j/k: select   t: transfer   r: rebalance   R: reload   b/Esc/q: home");
      end if;
      Curses.Refresh;
   end Draw;
      ...
      while Running loop
         ...
         begin
            ...
            then
               ...
               declare
                  Seed : constant Capacity_View := Execute (Current_Paths);
                  From_Seed : constant String :=
                    (if Seed.Success and then Seed.Count > 0
                       and then Cursor <= Seed.Count
                     then Coord_Name (Seed.Rows (Cursor).Coord)
                     else "");
                  Done : Boolean := False;
               begin
                  Run_Transfer
