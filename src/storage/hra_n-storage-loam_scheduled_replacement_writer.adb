...
package body HRA_N.Storage.Loam_Scheduled_Replacement_Writer is
   ...
   function Make_Failure
     (Status  : Scheduled_Replacement_Publish_Status;
   ...
   function Make_Success (Successor_Id : Scheduled_Id) return Publish_Result is
      Result : Publish_Result (Success => True);
   begin
      Result.Replacement_Id := Successor_Id;
      return Result;
   end Make_Success;
   ...
   begin
      ...
      then
         return Make_Failure
           (Invalid_Occurrence_Date,
            "Scheduled replacement requires a valid occurrence date");
      elsif not Equal_Token
        (Draft.Measure.Token, Make_Token ("jpy"))
      then
         return Make_Failure
           (Unsupported_Measure,
            "Scheduled replacement currently requires Measure jpy");
      elsif Draft.Changes.Count = 0 then
         return Make_Failure
           (Empty_Changes, "Scheduled replacement requires at least one change");
      end if;
                  ...
                  if Fresh_Text'Length = 0 then
                     return Fail
                       (Fresh_Identity_Exhausted,
                        "could not allocate fresh Scheduled identity");
                  end if;
                     ...
                     begin
                        ...
                        begin
                           ...
                           begin
                              ...
                              Release_All;
                              return Make_Success (Successor.Id);
                           end;
                        end;
                     end;
                  end;
      ...
      exception
         when others =>
            return Fail
              (Internal_Error,
               "unexpected LOAM Scheduled replacement writer failure");
      end;
   ...
   begin
      if Result.Success then
         ...
         case Result.Status is
            ...
            when Invalid_Source_Token =>
               return "Scheduled replacement requires a valid source identity";
            when Invalid_Occurrence_Date =>
               return "Scheduled replacement requires a valid occurrence date";
            when Unsupported_Measure =>
               return "Scheduled replacement currently requires Measure jpy";
            when Empty_Changes =>
               return "Scheduled replacement requires at least one change";
            when Invalid_Change_Locus_Or_Amount =>
               return "Scheduled replacement requires valid Locus tokens and nonzero quantities";
            when Changes_Not_Conserved =>
               return "Scheduled replacement changes must conserve exactly";
            when Lock_Failure =>
            ...
            when Source_Not_Retained =>
               return "selected Scheduled identity is not retained";
            when Locus_Not_Approved =>
               return "Scheduled replacement uses a Locus not approved for new publication";
            when Already_Completed =>
               return "selected Scheduled identity is already completed";
            when Interrupted_Completion =>
               return "selected Scheduled identity has an interrupted completion; retry completion before replacement";
            when Fresh_Identity_Exhausted =>
               return "could not allocate fresh Scheduled identity";
            when Source_Lifecycle_Invalid =>
               return "current Scheduled replacement lifecycle is invalid";
            when Working_Set_Exceeded =>
               return "HRA-N Scheduled replacement working-set capacity exceeded";
            when Source_Not_Current_Open =>
               return "selected Scheduled identity is no longer current-open";
            when Occurrence_Rejected =>
               return "replacement successor occurrence was rejected";
            when Unexpected_Transition_State =>
               return "unexpected Scheduled replacement transition state";
            when Scheduled_Insertion_Boundary_Absent =>
               return "Scheduled section insertion boundary is absent";
            when Replacement_Insertion_Boundary_Absent =>
               return "Replacement section insertion boundary is absent";
            when Candidate_Correspondence_Failure =>
               return "candidate Scheduled replacement failed semantic correspondence";
            when Staging_Write_Failure =>
               return "cannot durably stage canonical Scheduled replacement candidate";
            when Staging_Mismatch =>
               return "staged Scheduled replacement bytes do not match candidate generation";
            when Staging_Admission_Failure =>
               return "staged Scheduled replacement failed semantic admission";
            when Authority_Switch_Failure =>
               return "failed to switch canonical Scheduled replacement authority";
            when Internal_Error =>
               return "unexpected LOAM Scheduled replacement writer failure";
         end case;
      end if;
   end Format_Error;
